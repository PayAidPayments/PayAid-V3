...
import { CrmHandoffFields } from './CrmHandoffFields'
...
function NewProjectPageContent() {
  ...
  const handoffFromCrm = searchParams.get('source') === 'crm' || Boolean(prefillDealId)
  const [crmDealId, setCrmDealId] = useState('')
  const [handoffLabels, setHandoffLabels] = useState({ contactLabel: '', dealLabel: '' })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (flowProfile === 'agency' && !formData.clientId.trim()) {
      window.alert('Agency / Support profile requires a CRM contact before create.')
      return
    }
    ...
    createProject.mutate({
      ...
      ...(crmDealId ? { dealId: crmDealId, crmSource: 'crm' as const } : {}),
      ...(handoffFromCrm && !crmDealId ? { crmSource: 'crm' as const } : {}),
      ...(serviceTemplateId
        ? {
            serviceTemplateId,
  ...
  return (
    <div className="p-6">
      <div className="max-w-3xl mx-auto space-y-6">
        {handoffFromCrm ? (
          <div className="rounded-lg border border-purple-200 bg-purple-50 px-4 py-3 text-sm text-purple-900 dark:border-purple-800 dark:bg-purple-950/40 dark:text-purple-100">
            Creating from CRM — contact and deal are prefilled when available. Confirm selections in Commercial Setup.
          </div>
        ) : null}
        {/* Header */}
              ...
              {currentStep === 1 ? (
                <div className="space-y-4">
                  ...
                  <CrmHandoffFields
                    ...
                    dealId={crmDealId}
                    onClientIdChange={(clientId) => setFormData((prev) => ({ ...prev, clientId }))}
                    onDealIdChange={setCrmDealId}
                    onLabelsChange={setHandoffLabels}
                  ...
                  <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40 p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-700 dark:text-gray-300">
                      ...
                      <p>
                        ...
                        {handoffLabels.contactLabel || formData.clientId.trim() || 'Not set'}
                      </p>
                      <p>
                        <span className="font-medium">CRM deal:</span>{' '}
                        {handoffLabels.dealLabel || crmDealId || 'Not linked'}
                      </p>
                      <p>
...
export default function NewProjectPage() {
  return (
    <Suspense fallback={<PageLoading message="Loading project setup..." fullScreen={false} />}>
