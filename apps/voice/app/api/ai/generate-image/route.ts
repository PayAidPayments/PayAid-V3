...
export async function POST(request: NextRequest) {
  try {
    ...
    const payAidWorkerTroubleshoot =
      'PayAid image worker (IMAGE_WORKER_URL): start the service in services/text-to-image (see README), ensure POST /generate returns image_url, and that the app can reach the URL (same machine: http://localhost:7860).'
      ...
      setupInstructions: {
        ...
        payAidImageWorker: {
          steps: [
            ...
            '2. Set IMAGE_WORKER_URL=http://localhost:7860 in the same env as the app server',
