...
export default function TaxDeclarationsPage() {
  ...
  if (isLoading) {
    return <PageLoading message="Loading tax declarations..." fullScreen={false} />
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">Manage employee tax declarations</p>
        </div>
        <Link href={`/hr/${tenantId}/Tax-Declarations/New`}>
          <Button className="dark:bg-gray-700 dark:text-gray-100 dark:hover:bg-gray-600">New Declaration</Button>
        </Link>
      </div>
      ...
      <Card className="dark:bg-gray-800 dark:border-gray-700">
        <CardHeader>
          <CardTitle className="dark:text-gray-100">All Tax Declarations</CardTitle>
          <CardDescription className="dark:text-gray-400">{pagination?.total || 0} total declarations</CardDescription>
        </CardHeader>
        <CardContent>
          {declarations.length === 0 ? (
            <div className="text-center py-12 text-gray-500 dark:text-gray-400">
              ...
              <Link href={`/hr/${tenantId}/Tax-Declarations/New`}>
                <Button className="dark:bg-gray-700 dark:text-gray-100 dark:hover:bg-gray-600">Create First Declaration</Button>
              </Link>
            </div>
          ) : (
            <>
              <Table>
                <TableHeader>
                  <TableRow className="dark:border-gray-700">
                    ...
                    <TableHead className="dark:text-gray-300">Financial Year</TableHead>
                    <TableHead className="dark:text-gray-300">Declared Amount</TableHead>
                    <TableHead className="dark:text-gray-300">Approved Amount</TableHead>
                    <TableHead className="dark:text-gray-300">Proofs</TableHead>
                ...
                <TableBody>
                  {declarations.map((decl) => (
                    <TableRow key={decl.id} className="dark:border-gray-700 dark:hover:bg-gray-700/50">
                      ...
                      <TableCell className="dark:text-gray-200">{decl.financialYear}</TableCell>
                      <TableCell className="dark:text-gray-200">₹{Number(decl.declaredAmountInr).toLocaleString('en-IN')}</TableCell>
                      <TableCell className="dark:text-gray-200">
                        {decl.approvedAmountInr
                          ? `₹${Number(decl.approvedAmountInr).toLocaleString('en-IN')}`
                          : '-'}
                      </TableCell>
                      <TableCell className="dark:text-gray-200">{decl._count.proofDocuments}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Link href={`/hr/${tenantId}/Tax-Declarations/${decl.id}`}>
