...
// Opt-in only (exact `1`). Do not accept `true` — some shells/tooling set that and forced Turbopack.
const useTurbo =
...
// Next only needs CLI flags; dropping these avoids duplicate/confusing bundler hints in child.
