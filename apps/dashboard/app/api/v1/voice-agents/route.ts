// @ts-nocheck
/**
 * Voice Agents API (dashboard app)
 * Same contract as apps/voice — serves /api/v1/voice-agents when running `npm run dev` for dashboard.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user.tenantId) {
      return NextResponse.json({ error: 'No tenant ID found. Please log in again.' }, { status: 400 })
    }
...
const VOICE_AGENTS_LIST_TIMEOUT_MS = 28_000
...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: { tenantId: string; status?: string; language?: string } = { tenantId: effectiveTenantId }
    ...
    let overview:
      | { totalAgents: number; totalCalls: number; totalMinutes: number; conversionRate: number }
      | undefined
    if (includeStats) {
    ...
    return NextResponse.json(
      {
        error: isTimeout ? 'Database temporarily unavailable. Please try again.' : 'Failed to list agents',
        details: msg,
      },
