  ...
  try {
    ...
    const emailAccount = await prisma.emailAccount.upsert({
      ...
      create: {
        ...
        providerAccountId: userInfo.id || null,
        providerCredentials: providerCredentials as any,
        password: ['2a', '10', 'PLACEHOLDER_FOR_GMAIL_OAUTH'].join(String.fromCharCode(36)), // Placeholder hash-like token; Gmail uses OAuth
        isActive: true,
        lastSyncAt: new Date(),
      },
    })
    ...
    return NextResponse.redirect(
      new URL(`/settings/${tenantId}/Integrations/Email?success=gmail_connected&email=${encodeURIComponent(emailAddress)}`, request.url)
    )
  } catch (error) {
