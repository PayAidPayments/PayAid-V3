...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!appId || !appSecret) {
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/settings/${tenantId}/Integrations/Social?error=facebook_not_configured`)
    }
    ...
    if (!tokenRes.ok || !tokenJson?.access_token) {
      throw new Error(tokenJson?.error?.message || 'Failed to obtain Facebook access token')
    }
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/settings/${tenantId}/Integrations/Social?success=facebook_connected`)
  } catch (error) {
