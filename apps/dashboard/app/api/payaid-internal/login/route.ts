...
/** Explicit payload so `user.tenant` types correctly (Prisma Proxy can lose inference). */
...
async function handleLogin(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[LOGIN] Ignoring non-JSON or empty POST body (preflight/tooling):', {
        error: parseError instanceof Error ? parseError.message : String(parseError),
      })
      return NextResponse.json(
        { error: 'Invalid request body', message: 'Expected JSON with email and password.' },
    ...
    try {
      ...
      if (errorCode === 'P1001' || errorMessage.includes('Can\'t reach database server')) {
        ...
        // Column missing vs Prisma schema — do not confuse with "table not found"
        throw new Error(
          'Database is missing required columns (schema out of date). From repo root run: npx prisma migrate deploy'
        )
      } else if (errorCode === 'P2021') {
        throw new Error(
          'Database is missing a required table (schema out of date). From repo root run: npx prisma db push'
        )
    ...
    const effectiveModules = user.tenant?.moduleLicenses?.length
      ? user.tenant.moduleLicenses.map((m: { moduleId: string }) => m.moduleId)
      : (user.tenant?.licensedModules || [])
    ...
    return NextResponse.json({
      ...
      tenant: user.tenant ? {
        ...
        trialStartAt: user.tenant.trialStartAt,
    ...
    if (errorMessage.includes('temporarily unavailable') || errorMessage.includes('CIRCUIT_OPEN') || errorMessage.includes('high load')) {
      ...
      // In dev, show the real message (pool timeout, P2022, etc.); prod stays generic
      responseMessage = isDevelopment()
        ? errorMessage
        : 'Database connection error. Please try again in a moment. If the problem persists, contact support.'
