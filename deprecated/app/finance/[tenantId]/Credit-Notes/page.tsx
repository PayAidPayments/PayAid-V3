...
import { FileText, IndianRupee, Plus, ArrowLeftRight } from 'lucide-react'
...
export default function FinanceCreditNotesPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['credit-notes', tenantId, page, statusFilter],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/finance/credit-notes?${params.toString()}`, {
        headers: {
  ...
  if (isLoading) {
    return <PageLoading message="Loading credit notes..." fullScreen={false} />
  }
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <p className="text-red-700 dark:text-red-300">Error loading credit notes: {(error as Error).message}</p>
        </div>
      </div>
    )
  }
  ...
  const heroMetrics = [
    ...
    },
    {
      ...
      icon: <FileText className="w-5 h-5" />,
      href: statusFilter === 'draft' ? '#' : `/finance/${tenantId}/Credit-Notes?status=draft`,
    },
    {
      label: 'Issued',
      value: String(summary.issued),
      icon: <ArrowLeftRight className="w-5 h-5" />,
      href: statusFilter === 'issued' ? '#' : `/finance/${tenantId}/Credit-Notes?status=issued`,
    },
    {
      label: 'Cancelled',
      value: String(summary.cancelled),
      icon: <FileText className="w-5 h-5" />,
      href: statusFilter === 'cancelled' ? '#' : `/finance/${tenantId}/Credit-Notes?status=cancelled`,
    },
  ]
    ...
    <div className="w-full bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 relative" style={{ zIndex: 1 }}>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Credit Notes</h2>
            <p className="text-sm text-muted-foreground mt-1">
              Manage credit notes for returns, discounts, and adjustments
            </p>
          </div>
              ...
              {creditNotes.length === 0 ? (
                ...
                creditNotes.map((note: any) => (
                  <TableRow key={note.id}>
                    ...
                    <TableCell>{format(new Date(note.creditNoteDate), 'MMM dd, yyyy')}</TableCell>
                    ...
                    <TableCell>
                      <Link href={`/finance/${tenantId}/Credit-Notes/${note.id}`}>
