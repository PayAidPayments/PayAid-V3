...
// Two open deals: one closing this month, one beyond horizon
const NOW = new Date()
const thisMonthClose = new Date(NOW.getFullYear(), NOW.getMonth(), NOW.getDate() + 5)
...
describe('GET /api/v1/revenue/forecast (M3.1 smoke)', () => {
  ...
  it('200 — returns summary with best_case, most_likely, commit totals', async () => {
    const req = makeReq()
  ...
  it('200 — commit includes only deals with probability >= 70%', async () => {
    const req = makeReq()
    const res = await getForecast(req)
    const body = await res.json()
  ...
  it('200 — deals are bucketed by close date', async () => {
    const req = makeReq()
    const res = await getForecast(req)
    const body = await res.json()
  ...
  it('200 — pipeline_gap computed when target_inr supplied', async () => {
  ...
  it('200 — pipeline_gap is null when no target supplied', async () => {
    const req = makeReq()
    const res = await getForecast(req)
    const body = await res.json()
  ...
  it('200 — empty pipeline returns zeros', async () => {
    const db = require('@/lib/db/prisma')
  ...
  it('200 — window_days capped at 365', async () => {
  ...
  it('403 — FEATURE_DISABLED when m1_revenue_intelligence is off', async () => {
    const flags = require('@/lib/feature-flags/tenant-feature')
