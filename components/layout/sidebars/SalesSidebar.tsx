...
// Sales Module Navigation - Only Sales features
const salesNavigation = [
  ...
  { name: 'Orders', href: '/dashboard/orders', icon: '🛒', module: 'sales' },
]
...
export function SalesSidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  // Filter to show only licensed Sales items
  const licensedNav = salesNavigation.filter(item => 
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 w-64 overflow-y-auto relative transition-colors">
      ...
      <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200 dark:border-gray-700">
        <h1 className="text-xl font-bold text-blue-600 dark:text-blue-400">Sales</h1>
        <button
          onClick={onClose}
