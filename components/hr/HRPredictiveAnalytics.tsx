...
interface HRPredictiveAnalyticsProps {
  tenantId?: string
  stats?: any
}

export function HRPredictiveAnalytics({ tenantId, stats }: HRPredictiveAnalyticsProps) {
      ...
      try {
        ...
        if (stats) {
          ...
          if (stats.headcount > 0) {
            const predictedHeadcount = Math.round(stats.headcount * 1.05) // 5% growth
            mockPredictions.push({
              id: '1',
              metric: 'Headcount',
              current: stats.headcount,
          ...
          if (stats.nextPayrollAmount > 0) {
            const predictedPayroll = Math.round(stats.nextPayrollAmount * 1.08) // 8% growth
            mockPredictions.push({
              id: '2',
              metric: 'Payroll Cost',
              current: stats.nextPayrollAmount,
              predicted: predictedPayroll,
              change: 8,
              confidence: 75,
              period: 'Next Month',
            })
          ...
          if (stats.avgEngagement > 0) {
            const predictedEngagement = Math.min(Math.round(stats.avgEngagement * 1.03), 100) // 3% improvement
            mockPredictions.push({
              id: '3',
              metric: 'Engagement Score',
              current: stats.avgEngagement,
  ...
  const formatValue = (value: number, metric: string) => {
    if (metric.toLowerCase().includes('payroll') || metric.toLowerCase().includes('cost')) {
      return formatINRForDisplay(value)
    }
    if (metric.toLowerCase().includes('engagement')) {
      return `${value}/100`
    }
