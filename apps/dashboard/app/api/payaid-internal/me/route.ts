...
const meUserSelect = Prisma.validator<Prisma.UserSelect>()({
  id: true,
  email: true,
  name: true,
  role: true,
...
type MeUserData = Prisma.UserGetPayload<{ select: typeof meUserSelect }>
...
export async function GET(request: NextRequest) {
  try {
    ...
    const userData = await prismaWithRetry<MeUserData | null>(
      () =>
    ...
    const effectiveModules = userData.tenant?.moduleLicenses?.length
      ? userData.tenant.moduleLicenses.map((m: { moduleId: string }) => m.moduleId)
      : (userData.tenant?.licensedModules || [])

    if (userData.tenant) {
      const { moduleLicenses: _ml, ...tenantBase } = userData.tenant
      userData.tenant = {
        ...tenantBase,
        licensedModules: effectiveModules,
        subscriptionTier: userData.tenant.subscriptionTier || 'free',
      } as MeUserData['tenant']
    }
