...
export default function CRMHomeDashboard() {
  ...
  const fetchDashboardStatsRef = useRef<((signal?: AbortSignal, retryCount?: number) => Promise<void>) | null>(null)
  ...
  useEffect(() => {
    if (!allowDemoAutoSeed || !tenantId || !token || !stats || hasTriggeredEnsureDemoRef.current) return
        ...
        if (res.ok) {
          ...
          if (json.created?.deals > 0 || json.created?.tasks > 0) {
            if (abortControllerRef.current && !abortControllerRef.current.signal.aborted) {
              ...
              if (run) {
                run(abortControllerRef.current.signal)
              }
            }
          }
        }
  ...
  }, [allowDemoAutoSeed, tenantId, token, stats])
  ...
  useEffect(() => {
    if (!allowDemoAutoSeed || !tenantId || !token || hasCheckedDataRef.current) return
  ...
  }, [allowDemoAutoSeed, tenantId, token]) // Only run when tenantId or token changes
  ...
  const fetchDashboardStats = useCallback(async (signal?: AbortSignal, retryCount = 0): Promise<void> => {
    const MAX_RETRIES = 2
              ...
              onChange={(e) => {
                ...
                switch(view) {
                  ...
                  case 'sales':
                    ...
                    break
                  case 'pipeline':
                    router.push(`/crm/${tenantId}/Deals?view=pipeline`)
                    break
                  case 'activity':
                              ...
                              {activity.contact && (
                                <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                  ...
                                  {activity.contact.company && (
                                    <span className="text-gray-500">&middot; {typeof activity.contact.company === 'string' ? activity.contact.company : activity.contact.company.name}</span>
                                  )}
                                </div>
                              )}
                              {activity.metadata && (
        ...
        <div className="dashboard-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4" style={{ gridColumn: '1 / -1' }}>
            ...
            <Link href={tenantId ? `/crm/${tenantId}/Deals?category=won&timePeriod=${timePeriod}` : '#'} className="block">
              <StatCard
                title="Revenue"
                value={safeStats.revenueThisMonth ? formatINRForDisplay(safeStats.revenueThisMonth) : formatINRForDisplay(0)}
                trend={
                  <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                    <TrendingUp className="h-3.5 w-3.5 shrink-0" aria-hidden />
