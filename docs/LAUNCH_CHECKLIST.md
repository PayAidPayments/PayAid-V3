...
- [ ] 14) Auth speed baseline: run `npm run collect:crm-auth-baseline` (or `npm run check:crm-closure-env` + `node scripts/crm-auth-speed-sample.mjs`) and complete Day-0 item 4b + speed log row (latest artifacts: local host probes `2026-04-15T11-21-52-930Z` and `2026-04-15T11-22-43-856Z` show endpoint aborts; stable deployment probes `2026-04-15T11-47-46-975Z` and `2026-04-15T11-52-02-618Z` pass login and capture contacts/deals p95, but tasks remains blocked in current deployment (`/api/tasks` `404`, `/api/crm/tasks` `500`) with runtime evidence `docs/evidence/closure/2026-04-15T11-53-30-000Z-crm-auth-baseline-vercel-runtime-log-notes.md`; latest build blocker is Turbopack/Bull module resolution (`docs/evidence/closure/2026-04-15T19-04-38-996Z-vercel-turbopack-bull-build-blocker-notes.md`) after prior OOM/install-stage failures; install-command iteration evidence (`docs/evidence/closure/2026-04-15T13-46-00-000Z-crm-auth-baseline-vercel-install-command-iteration-notes.md`) confirms `npm ci` lockfile EUSAGE on Vercel and local `npm install` tarball-pack stall on this workstation; latest local rerun artifact `docs/evidence/closure/2026-04-15T12-43-24-144Z-crm-auth-baseline-run.md` confirms localhost health probe aborts and no sampler output).
...
| Auth speed baseline | Blocked | Stable deployment login now succeeds using newly registered tenant user, but tasks sampler still fails in current deployment (`/api/tasks` 404, `/api/crm/tasks` 500); latest local rerun still shows host health probe aborts and no sampler output. Deployment pipeline remains unstable: earlier OOM/install-stage failures, Turbopack/Bull module-resolution failure, plus install-command iteration failures (`npm ci` lockfile EUSAGE + local tarball stall). | Phani | Re-run production deploy with Bull externalization + Turbopack->Webpack fallback from a deployment-capable environment, then rerun `collect:crm-auth-baseline` and record p95 JSON for all three endpoints |
