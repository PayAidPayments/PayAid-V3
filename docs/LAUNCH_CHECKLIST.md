...
| Auth speed baseline | Blocked | Stable deployment login now succeeds using newly registered tenant user, but tasks sampler still fails in current deployment (`/api/tasks` 404, `/api/crm/tasks` 500); latest local rerun still shows host health probe aborts and no sampler output, and latest production deploys fail with OOM/SIGKILL (`dpl_64a16DK8XjSyxGQRhF9EfAJTVG1N`, `dpl_82CJJ37x6B8sET28fCWmi9wt2JA1`) | Phani | Complete successful production build+deploy of `/api/crm/tasks` native-handler fix (latest Vercel run uploads but build is OOM/SIGKILL), rerun `collect:crm-auth-baseline`, then record p95 JSON for all three endpoints |
