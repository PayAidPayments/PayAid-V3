...
- [ ] **Clear runtime blockers** for final evidence capture *(queue #13 is cleared with passing artifact `docs/evidence/closure/2026-04-15T11-01-56-039Z-crm-closure-blockers.md`; queue #14 now logs in successfully on stable env via a newly registered test tenant and captures contacts/deals p95, but deployed tasks sampling remains blocked pending redeploy verification (`/api/tasks` -> `404`, `/api/crm/tasks` -> `500` in current prod build). Upload-size blocker is now reduced (deployment archive dropped to 7,718 files), but latest production build still exits with SIGKILL/OOM during `next build` (see `docs/evidence/closure/2026-04-15T18-16-36-504Z-vercel-build-oom-notes.md`); Vercel-focused build-memory mitigations are now coded (`apps/dashboard/scripts/next-build.cjs`, `apps/dashboard/next.config.mjs`) and require redeploy verification.)*.
