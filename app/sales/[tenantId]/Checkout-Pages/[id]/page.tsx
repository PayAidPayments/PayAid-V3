...
export default function SalesCheckoutPageDetailPage() {
  ...
  const { data: page, refetch } = useQuery<CheckoutPage>({
    queryKey: ['sales-checkout-page', id],
    queryFn: async () => {
      const response = await fetch(`/api/sales/checkout-pages/${id}`, {
        headers: {
  ...
  if (!page) {
    return <PageLoading message="Loading checkout page..." fullScreen={false} />
  }
              ...
              <CardContent>
                {page.contentJson && page.contentJson.type === 'checkout-page' ? (
                  <div className="space-y-4 border rounded-lg p-6 bg-gray-50">
                    {page.contentJson.header && (
                      <div className="bg-white p-4 rounded border-b">
                        <h3 className="text-xl font-bold">{page.contentJson.header.title || page.name}</h3>
                      </div>
                    )}
                    ...
                    <div className="bg-white p-4 rounded">
                      <p className="text-sm text-gray-600 text-center">Payment methods: {Object.entries(page.paymentMethods || {})
                        .filter(([_, enabled]) => enabled)
                        .map(([key]) => key.toUpperCase())
                        .join(', ')}</p>
                    </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded">
                    <p className="text-sm text-gray-500">No content preview available. Checkout page content will be displayed here once configured.</p>
                  </div>
                )}
              </CardContent>
            </Card>
