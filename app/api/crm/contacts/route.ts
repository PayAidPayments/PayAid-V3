...
/**
 * CRM Module - Contacts API
 * Wrapper around existing /api/contacts with API Gateway integration
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Forward to existing contacts API
    const searchParams = request.nextUrl.searchParams;
    const response = await fetch(`${API_BASE_URL}/api/contacts?${searchParams.toString()}`, {
      headers: {
