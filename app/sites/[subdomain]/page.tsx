...
export default async function PublicWebsitePage({
  ...
  if (website.pages.length === 0) {
    ...
    console.log('💡 Tip: Make sure to publish at least one page for the website to be visible')
    notFound()
  }

  const currentPath = '/'
  const currentPage = website.pages.find((p) => p.path === currentPath) || website.pages[0]

  if (!currentPage) {
    console.log('❌ No page found for path:', currentPath)
    console.log('📄 Available pages:', website.pages.map(p => ({ path: p.path, title: p.title, published: p.isPublished })))
    notFound()
  }
