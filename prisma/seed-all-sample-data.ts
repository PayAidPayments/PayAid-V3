...
async function seedAllSampleData() {
  ...
  await seedNewsItems(tenantId, tenant.industry || 'all')
...
async function seedSocialMediaPosts(tenantId: string) {
  ...
  const posts = [
    // Facebook Posts
    {
      ...
      imageUrl: 'https://via.placeholder.com/800x600?text=Product+Launch',
      status: 'published',
    ...
    },
    {
      ...
      imageUrl: 'https://via.placeholder.com/800x600?text=Flash+Sale',
      status: 'published',
    ...
    },
    {
      ...
      imageUrl: 'https://via.placeholder.com/1200x630?text=Case+Study',
      status: 'published',
    ...
    // Instagram Posts
    {
      ...
      imageUrl: 'https://via.placeholder.com/1080x1080?text=Product+Photo',
      status: 'published',
      publishedAt: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000),
      engagement: 456,
      reach: 3800,
    ...
    },
    {
      ...
      imageUrl: 'https://via.placeholder.com/1080x1080?text=Team+Photo',
      status: 'published',
      publishedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
      engagement: 289,
      reach: 2400,
  ...
  for (const post of posts) {
    await prisma.socialPost.create({
      data: {
        ...post,
        tenantId,
        // imageUrl already set in post object
      },
    })
  }
...
async function seedWebsiteChatbots(tenantId: string, contactIds: string[]) {
  ...
  const chatbots = [
    ...
    },
    {
      ...
      greetingMessage: 'Need help with our products? Ask me anything!',
      autoGreet: false,
      isActive: true,
    },
    {
      ...
      position: 'bottom-right',
      greetingMessage: 'Interested in our services? Let\'s chat!',
      autoGreet: true,
      isActive: true,
    },
  ]
...
// ===== EXTENDED MODULES SEEDING FUNCTIONS =====
// Imported from seed-extended-modules.ts
 ...
 */
async function seedNewsItems(tenantId: string, industry: string) {
  const now = new Date()
  const sampleNews = [
    // Critical News
    {
      ...
      urgency: 'critical',
      icon: '🔴',
      industry: industry === 'restaurant' ? 'restaurant' : 'all',
      businessImpact: {
        what: 'GST on food services reduced from 5% to 0%',
        when: 'Effective February 1, 2026',
        where: 'All of India',
        who: 'Affects your business (restaurant)',
    ...
    // Important News
    {
      ...
      summary: 'Major competitor "Biryani Place" opens new branch in Banjara Hills',
      description: 'Your competitor Biryani Place has opened a new branch in Banjara Hills, just 2 km from your location. This could impact your market share.',
      source: 'competitor',
      ...
      businessImpact: {
        ...
        where: 'Banjara Hills, 2 km from your location',
      ...
      recommendedActions: [
        { action: 'Launch counter-promotion in that area', priority: 'high', deadline: '2026-01-20' },
    ...
    },
    {
      ...
      summary: 'Petrol and diesel prices have increased by 3% this week, affecting logistics costs',
    ...
    // Opportunity News
    {
      ...
      summary: 'New ₹50,000 grant available for small businesses in food service sector',
      description: 'The government has announced a new grant program offering ₹50,000 to small businesses in the food service sector. Applications open until March 31, 2026.',
      source: 'government',
      category: 'market-trends',
      urgency: 'opportunity',
      icon: '💡',
      ...
      businessImpact: {
        ...
        where: 'Nationwide',
        who: 'Small businesses in food service sector',
    ...
    },
    {
      ...
      sourceName: 'Economic Times',
      publishedAt: new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000), // 14 days ago
  ...
  // Check if general news already exists
  const existingGeneralNews = await prisma.newsItem.findFirst({
    where: {
      tenantId: null,
      title: generalNews[0].title,
    },
  })
