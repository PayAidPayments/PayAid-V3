#!/usr/bin/env node
/**
 * No-404 QA: Supervisor monitor page + API route.
 */
...
async function probePage() {
  const url = new URL(`${base.replace(/\/$/, '')}/voice-agents/${tenantId}/Monitor`)
  if (bypass) {
...
async function probeApi() {
  ...
  const url = new URL(`${base.replace(/\/$/, '')}/api/v1/voice-agents/supervisor/monitor`)
  if (bypass) {
    url.searchParams.set('x-vercel-set-bypass-cookie', 'true')
    url.searchParams.set('x-vercel-protection-bypass', bypass)
  }
