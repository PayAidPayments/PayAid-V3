...
if (!allowByEnv && !approvalActive) {
  console.error(
    JSON.stringify(
      {
        ok: false,
        error: 'Marker cleanup apply blocked: no active approval.',
        approvalActive,
        allowByEnv,
        status: statusParsed,
        nextSteps: [
          ...
          'Retry: npm run cleanup:leads-bulk-retention-artifacts:with-markers:preflight:apply',
        ],
      },
      null,
      2,
  ...
  JSON.stringify(
    {
      ...
      check: 'leads-bulk-retention-marker-cleanup-preflight',
      approvalActive,
      allowByEnv,
