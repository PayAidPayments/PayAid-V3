...
// Finance Module Navigation - Only Finance features
const financeNavigation = [
  { name: 'Dashboard', href: '/dashboard', icon: '📊', module: null },
  { name: 'Invoices', href: '/dashboard/invoices', icon: '🧾', module: 'finance' },
...
export function FinanceSidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  // Filter to show only licensed Finance items
  const licensedNav = financeNavigation.filter(item => 
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 w-64 overflow-y-auto relative transition-colors">
      ...
      <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200 dark:border-gray-700">
        <h1 className="text-xl font-bold text-blue-600 dark:text-blue-400">Finance</h1>
        <button
          onClick={onClose}
