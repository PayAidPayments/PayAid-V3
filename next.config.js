...
const nextConfig = {
  ...
  experimental: {
    ...
    // On Vercel: single process/CPU to avoid OOM. Locally: use defaults for much faster compiles.
    ...(process.env.VERCEL === '1'
      ? { webpackBuildWorker: false, cpus: 1 }
      : {}),
  },
    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://unpkg.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https://fonts.gstatic.com; media-src 'self' blob:; connect-src 'self' ws://localhost:3001 wss://localhost:3001 ws://127.0.0.1:3001 wss://127.0.0.1:3001 https://*.supabase.co https://*.vercel.app;"
          },
          {
            key: 'Referrer-Policy',
